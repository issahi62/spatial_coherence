...
myFolder = '/Users/kobbyTilly/Desktop/Msc(UEF)_Biophotonics/Third_Period/SecondLabWork/Spatial_coherence_IMS/13AmLD'; 
if ~isdir(myFolder)
...
%
vect = 1; 
filePattern = fullfile(myFolder, '13AmLD_*');
...
InverseFourier_Collector = abs(InverseFourier(selector:401, :));
AnotherSrijoyee = [(AnotherSrijoyee); sum(InverseFourier_Collector)./8];    
...
fname ='/Users/kobbyTilly/Desktop/Msc(UEF)_Biophotonics/Third_Period/SecondLabWork/Spatial_coherence_IMS/13AmLD_images';
...
for n = 1:length(matFiles) 
    Kplus(n, :) = [tilly(n,((staff+1)-n)*16+ShifterX:end), tilly(n, 1:((staff+1)-n)*16-1+ShifterY)];
end
finalimage =  [Kplus(:, 1555:end), Kplus(:, 1:1499)];
 mustafa = figure(101); 
 imagesc(finalimage)
